...
function hoot_appearance_subpage() {
	...
	add_theme_page(
		__( 'Juxter Premium', 'juxter' ),	// Page Title
		__( 'Premium Options', 'juxter' ),	// Menu Title
		'edit_theme_options',					// capability
		'juxter-welcome',						// menu-slug
...
function hoot_admin_enqueue_upsell_styles( $hook ) {
	if ( $hook == 'appearance_page_juxter-welcome' )
...
function hoot_theme_appearance_subpage() {
	...
	$hoot_cta = ( empty( $hoot_cta ) ) ? '<a class="button button-primary button-buy-premium" href="https://wphoot.com/" target="_blank">' . __( 'Click here', 'juxter' ) . '</a>' : $hoot_cta;
	...
	$hoot_intro = wp_parse_args( $hoot_intro, array(
		'name' => __('Upgrade to Premium', 'juxter'),
		'desc' => '',
		) );
	?>
