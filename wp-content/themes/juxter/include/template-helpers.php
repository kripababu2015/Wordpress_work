...
function hoot_get_logo_image() {
	...
	$class = '';
	$class = ( empty( $class ) ) ? '' : ' class="' . $class . '"';
	$tagline = ( hoot_get_mod( 'show_tagline' ) ) ? hybrid_get_site_description() : '';
	if ( !empty( $has_logo ) ) {
...
function hoot_header_aside() {
	?><div <?php hybridextend_attr( 'header-aside', '', 'header-aside-widget-area' ); ?>><?php
 ...
 * @return void
 */
if ( !function_exists( 'hoot_header_menu' ) ):
function hoot_header_menu() {
	$site_layout = hoot_get_mod( 'site_layout', 'boxed' );
	$show_nav_menu_home = hoot_get_mod( 'show_nav_menu_home' );
	$class = ( $site_layout == 'boxed' ) ? 'hgrid' : ''; // : 'hgrid-stretch';
	$class .= ( $show_nav_menu_home ) ? ' nav-with-homeicon' : ' nav-no-homeicon';
	?>
	<div <?php hybridextend_attr( 'header-part', 'supplementary', $class ); ?>>
		<div class="hgrid">
			<div class="hgrid-span-12">
				<?php
				$homeicon = apply_filters( 'hoot_menu_homeicon', '<div class="header-static-home"><a href="' . esc_url( home_url() ) . '" class="accent-typo"><i class="fas fa-home"></i></a></div>' );
				if ( $homeicon && $show_nav_menu_home && has_nav_menu( 'primary' ) )
					echo wp_kses_post( $homeicon );
...
function hoot_meta_info_blocks( $args = '', $context = '', $editlink = true ) {
	...
	if ( !empty( $display['author'] ) ) :
		$blocks['author']['label'] = __( 'By:', 'juxter' );
		ob_start();
	...
	if ( !empty( $display['date'] ) ) :
		$blocks['date']['label'] = __( 'On:', 'juxter' );
	...
	if ( !empty( $display['cats'] ) ) :
		...
		if ( !empty( $category_list ) ) :
			$blocks['cats']['label'] = __( 'In:', 'juxter' );
			$blocks['cats']['content'] = $category_list;
		endif;
	endif;

	if ( !empty( $display['tags'] ) && get_the_tags() ) :
		$blocks['tags']['label'] = __( 'Tagged:', 'juxter' );
		$blocks['tags']['content'] = ( ! get_the_tags() ) ? __( 'No Tags', 'juxter' ) : get_the_tag_list( '', ', ', '' );
	endif;

	if ( !empty( $display['comments'] ) && comments_open() ) :
		$blocks['comments']['label'] = __( 'With:', 'juxter' );
		ob_start();
		comments_popup_link(__( '0 Comments', 'juxter' ),
							__( '1 Comment', 'juxter' ),
							__( '% Comments', 'juxter' ), 'comments-link', '' );
		$blocks['comments']['content'] = ob_get_clean();
	endif;

	if ( $editlink && $edit_link = get_edit_post_link() ) :
		$blocks['editlink']['label'] = '';
		$blocks['editlink']['content'] = '<a href="' . $edit_link . '">' . __( 'Edit This', 'juxter' ) . '</a>';
	endif;
...
function hoot_display_404_content() {
	echo esc_html( __( 'Apologies, but no entries were found.', 'juxter' ) );
}
endif;
