...
function hybrid_archive_title_filter( $title ) {
	...
	elseif ( is_search() )
		...
		$title = sprintf( esc_html__( 'Search results for: %s', 'juxter' ), get_search_query() );

	elseif ( function_exists( 'bbp_is_single_user' ) && bbp_is_single_user() )
		$title = __( 'User Profile ', 'juxter' );
	...
	if ( ( ( $page = get_query_var( 'paged' ) ) || ( $page = get_query_var( 'page' ) ) ) && $page > 1 ) {
		...
		$suffix = ' <span class="loop-title-suffix loop-title-paged">' . sprintf( __( '(Page %s)', 'juxter' ), $paged ) . '</span>';
